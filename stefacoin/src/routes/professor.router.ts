...
router.delete('/professor/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if(professor.tipo === TipoUsuario.PROFESSOR){
      const mensagem: Mensagem = await new ProfessorController().excluir(Number(id));
      res.json(mensagem);
    }else{
      throw res.status(404).json({message : "Professor não encontrado"})
    }
...
router.get('/professor', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const PROFESSORES = professores.flatMap((professor, index) => {return [{id: professor.id, nome: professor.nome}]})
